 ...
 */
function toPktDate(utcIsoString: string | Date): Date {
  const utcMs = new Date(utcIsoString).getTime();
  return new Date(utcMs + PKT_OFFSET_MS);
}

export function formatPktDate(utcIsoString: string | Date): string {
...
export function formatPktDateTime(utcIsoString: string | Date): string {
  if (!utcIsoString) return '';
  const date = toPktDate(utcIsoString);
...
export function formatPktTime(utcIsoString: string | Date): string {
  if (!utcIsoString) return '';
  const date = toPktDate(utcIsoString);
 ...
 */
export function getPktNowForInput(): string {
  const now = new Date();
 ...
 */
export function pktInputToUtcIso(pktDatetimeLocal: string): string {
